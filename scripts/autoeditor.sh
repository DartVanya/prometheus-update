...
if [[ -z $(egrep "^CONFIG_FIRMWARE_INCLUDE_ARIA=y" -o ./$DIRC/routers/$ROUTERS.sh) ]]
then
   echo -e "$NONE 6. Aria2                    [0.0 MB] [$RED0$off$NONE]"
else
   echo -e "$NONE 6. Aria2                    [3.5 MB] [$GREEN0$on$NONE]"
fi
...
then
   echo -e "$NONE 7. Aria2 WEB control        [0.0 MB] [$RED0$off$NONE]"
else
   echo -e "$NONE 7. Aria2 WEB control        [0.7 MB] [$GREEN0$on$NONE]"
fi
...
then
   echo -e "$NONE 8. Alternative RP-L2TP      [0.0 MB] [$RED0$off$NONE]"
else
   echo -e "$NONE 8. Alternative RP-L2TP      [0.1 MB] [$GREEN0$on$NONE]"
fi
### Include ffmpeg for Minidlna. ~0.1MB
if [[ -z $(egrep "^CONFIG_FIRMWARE_INCLUDE_FFMPEG_NEW=y" -o ./$DIRC/routers/$ROUTERS.sh) ]]
then
   echo -e "$NONE 9. FFmpeg & INSTEAD         [0.0 MB] [$RED0$off$NONE]"
else
   echo -e "$NONE 9. FFmpeg & INSTEAD         [0.1 MB] [$GREEN0$on$NONE]"
fi
echo -e "$NONE--------------------------------------------------------------------"
...
EOF8
    ...
    "6")  ### Include Aria2 download manager. openssl ~1.2MB, aria2 ~3.5MB
          if [[ ! -z $(egrep "^CONFIG_FIRMWARE_INCLUDE_ARIA=y" -o ./$DIRC/routers/$ROUTERS.sh) ]]
          then
             sed -i "s/CONFIG_FIRMWARE_INCLUDE_ARIA=y/#CONFIG_FIRMWARE_INCLUDE_ARIA=y/" ./$DIRC/routers/$ROUTERS.sh
          else
          ...
          rssh ;;
    "7")  ### Include Aria2 WEB control. ~0.7MB
          if [[ ! -z $(egrep "^CONFIG_FIRMWARE_INCLUDE_ARIA_WEB_CONTROL=y" -o ./$DIRC/routers/$ROUTERS.sh) ]]
          then
          ...
          rssh ;;
    "8")  ### Include alternative L2TP control client RP-L2TP. ~0.1MB
          if [[ ! -z $(egrep "^CONFIG_FIRMWARE_INCLUDE_RPL2TP=y" -o ./$DIRC/routers/$ROUTERS.sh) ]]
          then
          ...
          rssh ;;
    "9")  ### Include ffmpeg for Minidlna. ~0.1MB
          if [[ ! -z $(egrep "^CONFIG_FIRMWARE_INCLUDE_FFMPEG_NEW=y" -o ./$DIRC/routers/$ROUTERS.sh) ]]
          then
             sed -i "s/CONFIG_FIRMWARE_INCLUDE_FFMPEG_NEW=y/#CONFIG_FIRMWARE_INCLUDE_FFMPEG_NEW=y/" ./$DIRC/routers/$ROUTERS.sh
          fi
